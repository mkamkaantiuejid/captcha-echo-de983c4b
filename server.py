...
from common.mistral_config import (
    DEFAULT_MODEL,
    add_keys,
    clear_keys,
    get_status,
...
# Docs recommend headed browsers (BROWSER_HEADLESS=0). Default to headed when unset so
# local/dev runs on Windows/macOS work without extra env — systemd sets this explicitly.
...
_TAGS = [
    ...
    {"name": "dashboard", "description": "Web dashboard with per-solver configuration tutorials."},
    {"name": "config", "description": "Mistral vision setup (models + key file status)."},
    {"name": "harvest", "description": "Harvest session cookies + form fields for caller replay."},
]
...
_SOLVE_EXAMPLES = {
    ...
    "arkose": {
        ...
        "value": {"type": "arkose", "public_key": "A0DE7B75-1138-44F2-B132-ED188CEB66F3",
                  "page_url": "https://login.databricks.com/login", "max_waves": 10},
    },
    "aliyun": {
        "summary": "Aliyun Captcha 2.0 slide-puzzle",
        "value": {"type": "aliyun", "scene_id": "1r7eif79x", "prefix": "13lbkb5", "region": "sgp"},
    },
}
...
class StatusResponse(BaseModel):
    ...
    browser_mode: str = Field(description="headed or headless (BROWSER_HEADLESS env)")
...
class MistralKeyHint(BaseModel):
    index: int
    hint: str = Field(description="Last 4 characters of the key (full keys never returned)")


class MistralConfigResponse(BaseModel):
    config_path: str
    keyfile_path: str
    config_exists: bool
    keyfile_exists: bool
    ...
    key_hints: list[MistralKeyHint] = Field(default_factory=list)
    models: dict[str, str]
    env_override_active: dict[str, bool]
    default_model: str
    setup: list[str]


class MistralKeysBody(BaseModel):
    keys: list[str] = Field(..., min_length=1, description="Mistral API keys (one or more)")


class MistralKeysResult(BaseModel):
    keys_count: int
    added: Optional[int] = Field(None, description="Keys appended (POST only)")
    key_hints: list[MistralKeyHint] = Field(default_factory=list)


class MistralConfigUpdate(BaseModel):
    recaptcha_model: str = Field(DEFAULT_MODEL, description="Vision model for reCAPTCHA image grids")
    hcaptcha_model: str = Field(DEFAULT_MODEL, description="Vision model for hCaptcha image grids")
    aliyun_model: str = Field(DEFAULT_MODEL, description="Vision model for Aliyun VLM gap detection")


class HarvestGithubSignupRequest(BaseModel):
    proxy: Optional[str] = Field(None, description="Per-request proxy (recommended for GitHub)")
    timeout_s: Optional[int] = Field(90, description="Max wait for DataDome silent pass on /signup")


@app.post("/harvest/github-signup", tags=["harvest"],
          summary="Harvest GitHub signup session (form fields + cookies + DataDome)")
async def harvest_github_signup_endpoint(req: HarvestGithubSignupRequest):
    """One headed browser pass on github.com/signup: intercept DataDome, scrape
    timestamp_secret / authenticity_token / honeypot name, return full cookie jar.

    Caller replays signup via HTTP (curl_cffi) with returned user_agent + cookies + proxy.
  DataDome blocks plain curl GET — this endpoint is the minimal browser step."""
    from github_signup.harvest import harvest_github_signup
    try:
        async with asyncio.timeout(req.timeout_s or 90):
            result = await harvest_github_signup(proxy=req.proxy, timeout_s=req.timeout_s or 90)
    except (TimeoutError, asyncio.TimeoutError):
        raise HTTPException(408, f"harvest timed out after {req.timeout_s or 90}s")
    if not result.get("success"):
        raise HTTPException(400, result.get("error") or "harvest failed")
    return result
...
@app.get("/config/mistral", response_model=MistralConfigResponse, tags=["config"],
         summary="Mistral vision setup status (models + apikey.txt)")
async def mistral_config_get():
    """Returns effective Mistral models and whether common/apikey.txt has keys.
    Configure models via PUT or edit common/mistral.json — no .env required."""
    return get_status()


@app.put("/config/mistral", response_model=MistralConfigResponse, tags=["config"],
         summary="Save Mistral vision models to common/mistral.json")
async def mistral_config_put(req: MistralConfigUpdate):
    """Writes recaptcha/hcaptcha/aliyun model names. API keys stay in common/apikey.txt."""
    save_models(req.recaptcha_model, req.hcaptcha_model, req.aliyun_model)
    return get_status()


@app.post("/config/mistral/keys", response_model=MistralKeysResult, tags=["config"],
          summary="Append Mistral API keys to common/apikey.txt")
async def mistral_keys_append(req: MistralKeysBody):
    """Add one or more keys (deduped). Full keys are never echoed back."""
    try:
        result = add_keys(req.keys)
    ...
    return {
        ...
        "added": result["added"],
        "key_hints": status["key_hints"],
    }


@app.put("/config/mistral/keys", response_model=MistralKeysResult, tags=["config"],
         summary="Replace all Mistral API keys in common/apikey.txt")
async def mistral_keys_replace(req: MistralKeysBody):
    """Replace the entire key pool. Use POST to append without removing existing keys."""
    result = replace_keys(req.keys)
    status = get_status()
    return {"keys_count": result["keys_count"], "key_hints": status["key_hints"]}


@app.delete("/config/mistral/keys", response_model=MistralKeysResult, tags=["config"],
            summary="Remove Mistral API keys")
async def mistral_keys_delete(index: Optional[int] = Query(
    None, description="Remove key at this index (from GET key_hints). Omit to clear all keys."
)):
    """Delete one key by index, or clear the entire pool when index is omitted."""
    try:
        if index is None:
            result = clear_keys()
        else:
            result = remove_key_at(index)
    except ValueError as e:
        raise HTTPException(400, str(e))
    status = get_status()
    return {"keys_count": result["keys_count"], "key_hints": status["key_hints"]}
...
async def dashboard_index():
    """Web dashboard — live monitoring + configuration tutorials for all solvers."""
...
async def solver_status():
    ...
    headed = os.getenv("BROWSER_HEADLESS", "0") == "0"
    return {
        "services": {t: "online" for t in SUPPORTED},
        "current": list(_solve_current.values()),
        "browser_mode": "headed" if headed else "headless",
    }
...
if __name__ == "__main__":
    ...
    # Single worker — browser solvers use per-type locks; do not scale with workers>1
    uvicorn.run(
        "server:app",
        host="0.0.0.0",
        port=port,
