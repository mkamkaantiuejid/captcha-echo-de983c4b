...
**Deploy:** see [DEPLOY.md](DEPLOY.md) for production install, systemd, and pre-flight checks.
...
| GET    | `/dashboard` | public    | **Web dashboard** — monitoring + per-solver config tutorials |
...
├── DEPLOY.md              # Production install guide
├── deploy/
│   ├── install.sh         # Linux installer → /opt/captcha-solver
│   ├── captcha-solver.service  # systemd unit (xvfb + headed browser)
│   └── env.optional.example    # Optional PORT/GEOIP overrides (not required)
├── run.sh / run.bat       # Dev launchers (BROWSER_HEADLESS=0)
...
│   ├── mistral_config.py  # models in mistral.json (no .env required)
│   ├── mistral.json.example
...
│   └── solve.py
├── github_signup/         # GitHub /signup harvest (POST /harvest/github-signup)
│   └── harvest.py
├── dashboard/             # Web UI at /dashboard
│   ├── index.html
│   └── static/
├── scripts/
│   ├── verify_deploy.py   # Pre-flight deploy checks
│   ├── test_github_register_api.py
│   └── test_github_register.py
